...
#     all_positions = cast_input(org_positions)
    ...
    for traji in range(0, all_positions.size(0)):
        for conf_id in range(1, all_positions[traji].size(1)):
            for i, pos in enumerate(all_positions[traji,conf_id]):
                conf.SetAtomPosition(i, pos)
            mol.AddConformer(conf, assignId=True)
            conf = Chem.Conformer(mol.GetNumAtoms())
        Chem.MolToPDBFile(mol, f"{fname}_traj_{traji}_confs.pdb")
...
if __name__ == "__main__":
    ...
    get_rdkit_mol(
            fname=filename,
            all_positions=data_dict["traj"] * 10,
